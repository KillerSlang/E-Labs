...
{    
    ...
    if(!(empty($_FILES["uploadtheorie"]) || $_FILES["uploadtheorie"]["error"] == 4))
    {
        // upload theorie
    ...
    if(!(empty($_FILES["uploadmaterialen"]) || $_FILES["uploadmaterialen"]["error"] == 4))
    {
        // upload materialen

        $fileType = pathinfo(basename($_FILES["uploadmaterialen"]["name"]), PATHINFO_EXTENSION); 
        $target_dir = "../upload/voorbereiding/materialen/Materialen ";
        $date = date("Y-m-d_G-i-s");
    ...
    if(!(empty($_FILES["uploadmethode"]) || $_FILES["uploadmethode"]["error"] == 4))
    {
        // upload Methode
    ...
    if(!(empty($_FILES["uploadveiligheid"]) || $_FILES["uploadveiligheid"]["error"] == 4))
    {
        // upload logboek
    ...
    if(!(empty($_FILES["uploadvoorbereidendevragen"]) || $_FILES["uploadvoorbereidendevragen"]["error"] == 4))
    {
        // upload voorbereidendevragen

        $fileType = pathinfo(basename($_FILES["uploadvoorbereidendevragen"]["name"]), PATHINFO_EXTENSION); 
        $target_dir = "../upload/voorbereiding/vragen/Voorbereidendevragen ";
        $date = date("Y-m-d_G-i-s");
    ...
    $hypothese, $materialen, $methode, $veiligheid, $voorbereidendevragen, $vak,$doel);
